...
def cg(a, b, M, reg, f, df, G0=None, numItermax=500, stopThr=1e-09,
       verbose=False,log=False,amijo=True,C1=None,C2=None,constC=None):
    """
...
def partial_cg(a, b, M, reg, m, nb_dummies=1, G0=None, thres=1,
               numItermax=500, stopThr=1e-07,
               verbose=False,log=False,amijo=True,
               C1=None,C2=None,constC=None):
    """
    Solve the general regularized OT problem with conditional gradient
        The function solves the following optimization problem:
    .. math::
    ...
    cC1 = np.dot(C1 ** 2 / 2, np.dot(G, np.ones(C2.shape[0]).reshape(-1, 1)))
    cC2 = np.dot(np.dot(np.ones(C1.shape[0]).reshape(1, -1), G), C2 ** 2 / 2)
    constC = cC1 + cC2
    ...
    while loop:
        ...
        Gcemd, logemd = emd(a_extended, b_extended, M_emd, log=True)

        if logemd['warning'] is not None:
            raise ValueError("Error in the EMD resolution: try to increase the"
                             " number of dummy points")

        if nb_dummies > 0 and np.any(Gcemd[-nb_dummies:, -nb_dummies:] > eps):
